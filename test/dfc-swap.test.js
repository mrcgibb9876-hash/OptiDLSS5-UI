...
test('the swap is offered for 64-bit Direct3D 9 to 12, and says why elsewhere', () => {
  ...
  for (const api of ['dx9', 'dx10', 'dx11', 'opengl', 'vulkan']) assert.strictEqual(dfc.supportedFor({ api, bitness: 32 }).ok, true, `32-bit ${api}`);
  for (const api of ['dx8', 'dx12']) assert.strictEqual(dfc.supportedFor({ api, bitness: 32 }).code, 'dfc-32bit', `32-bit ${api}`);
});
