...
const TOOLS = [
  {
    name: 'apply_fix',
    description: 'Run one of the app\'s own fixes on this game. remove-foreign: remove another DLSS 5 toolchain\'s files (DLSS5-Swapper, oneclick). remove-feeder: remove the DLSS5 Feeder stack. remove-luma: remove the Luma UE stack. redeploy-feeder: deploy the DLSS5 Feeder stack again, forced, rewriting the motion-vector provider (VORT by default -- the old DRME default cannot compile on ReShade 6.8 and fed no vectors), its shader, both DLSS5_MV_PROVIDER levels, the ReShade search paths and the preset. feeder-depth-profile: switch this game to the contributor-verified Unity depth profile, for a game whose depth reads flat. disable-agility-redist: move the game\'s own D3D12 (Agility SDK) folder aside, reversibly, when it is failing every D3D12 device create in the process -- the Feeder\'s own private device included. reconfigure: re-run the app\'s automatic configuration (OptiScaler.ini upscaler keys incl. dlss_12 for D3D11, NR enabled, REFramework for RE Engine, ReShade ini). remove-all: remove everything this app placed in the game folder and restore what it set aside (the card\'s Remove) -- only for a route that cannot run on this game, such as dgVoodoo2 crashing it at startup. install: run the card\'s Install for the recommended route (OptiScaler, Feeder or Luma as the route says). Returns what happened.',
    input_schema: {
      type: 'object',
      properties: {
        fix: { type: 'string', enum: ['remove-foreign', 'remove-feeder', 'remove-luma', 'redeploy-feeder', 'feeder-depth-profile', 'disable-agility-redist', 'reconfigure', 'remove-all', 'install'] },
