...
ipcMain.handle('game:install', async (_evt, { exePath, releaseFolder, nrDllPath }) => {
  try {
    ...
    if (nrDllPath) source.payload.push({ name: 'nvngx_dlssnr.dll', path: nrDllPath });

    const config = { gameDir: dir, exePath, api, optiRoot, source, profile: {} };

    checkConflicts(dir, exePath, null, api);

    const logLines = [];
    const manifest = await installOptiScaler(config, (entry) => logLines.push(entry));

    return { ok: true, dir, manifest, api, logs: logLines };
  } catch (err) {
    const message = err.code === 'errOptiConflict' ? `Mod conflict detected: ${err.message}` : err.message;
