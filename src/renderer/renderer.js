...
async function applyRecommendation(game, card, backends, generation = renderGeneration) {
  ...
  if (panelEl) {
    ...
    if (showPanelNote) {
      ...
      panelEl.title = t('A 32-bit game cannot run DLSS in its own process, so the neural pass runs in the 64-bit helper beside the game -- and OptiScaler\'s menu (Insert) and the DLSS 5 panel (Alt+Home) run there with it, in a process with no window. Both keys are correct; they are just being sent to the game. The Feeder\'s add-on has to draw the helper\'s window over the game first. Needs windowed or borderless. Game Help spells it out.');
    }
  }
...
function helpWords(diag) {
  ...
  switch (diag.code) {
    ...
    case 'ok-panel-in-helper': return t('DLSS 5 is working here: Neural Rendering ran {count} passes on the last run{fps}. Neither of OptiScaler\'s two menus will open over this game, and that is expected rather than a fault: this is the 32-bit route, and a 32-bit game cannot run DLSS in its own process, so the neural pass runs in the 64-bit helper beside it -- and both menus run there too, in a process with no window of its own. Insert (OptiScaler\'s own menu) and Alt+Home (the DLSS 5 panel) are the right keys; they are simply being sent to the game, which is not where either menu is. To reach them: press Home in the game for ReShade\'s overlay, go to the Add-ons tab, open DLSS 5 Feed, and press "Show the DLSS 5 panel in-game" -- that draws the helper\'s window over the game and passes your keys and clicks to it. It needs windowed or borderless; in exclusive fullscreen use the Feeder\'s "Show as texture" option instead. If that does not work either, nothing is lost: everything in the panel can be set from Edit here, with the game closed, and Game Help can save a bundle that now includes the helper\'s own log.', { count: v.count, fps: v.fps ? t(' at {fps} fps', { fps: v.fps }) : '' });
