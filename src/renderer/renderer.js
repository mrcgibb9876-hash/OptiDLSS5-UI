...
async function applyRecommendation(game, card, backends, generation = renderGeneration) {
  ...
  if (panelEl) {
    ...
    if (showPanelNote) {
      panelEl.textContent = t('Menus are in the helper: Home \u2192 Add-ons \u2192 DLSS 5 Feed \u2192 show the panel');
      panelEl.title = t('Expect nothing on screen in the game: no OptiScaler splash when it loads, no menu on Insert or Alt+Home. A 32-bit game cannot run DLSS in its own process, so the neural pass runs in the 64-bit helper beside the game -- and OptiScaler runs there with it, in a process with no window to draw on. Both keys are correct; they are just being sent to the game. The Feeder\'s add-on has to draw the helper\'s window over the game first. Needs windowed or borderless. Game Help spells it out.');
    }
  }
...
async function installGame(game) {
  ...
  if (route.route === 'feeder32') {
    ...
    toast(res32.ok
      ? t('Installed the experimental 32-bit route. Expect no OptiScaler splash in the corner and no menu on Insert or Alt+Home -- on this route OptiScaler runs in the 64-bit helper beside the game, not in the game, so it has no window there to draw on. It is still working; the card says so after a run. To reach its menus: Home opens ReShade -> Add-ons -> DLSS 5 Feed -> "Show the DLSS 5 panel in-game".')
