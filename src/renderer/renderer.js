...
function helpWords(diag) {
  ...
  switch (diag.code) {
    ...
    case 'sr-backend-fallback': return v.why
      ? t('DLSS could not be created and OptiScaler silently upscaled with {backend} instead. The neural pass still ran on top of it, which is why this looks like a working run -- but the game is not running DLSS. NVIDIA\x27s own reason for refusing is {result}, {why}. Send the report: nvngx.log beside the exe is NGX\x27s own log and is the only thing that says more than the code does -- the bundle collects it.', v)
